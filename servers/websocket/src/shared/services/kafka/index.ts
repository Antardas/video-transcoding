import { MessageType, TOPIC_TYPE, ProgressEvent } from './../../../types/subscription.type';
import { Consumer, Kafka, logLevel, Partitioners, Producer } from 'kafkajs';
import { KafkaType, MessageHandler, PublishType } from './kafka.type';

// Configuration
const CLIENT_ID = process.env.CLIENT_ID || 'websocket-service';
const GROUP_ID = process.env.GROUP_ID || 'websocket-service-group';
...
const connectProducer = async <T>(): Promise<T> => {
	await createTopic(['ProgressEvents']);
			...
			try {
				if (message.key && message.value) {
					const inputMessage: MessageType = {
						...
						event: message.key.toString() as ProgressEvent,
